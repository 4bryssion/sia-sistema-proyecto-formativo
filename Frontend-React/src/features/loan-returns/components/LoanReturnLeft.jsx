import { getLoanStatusLabel } from "@/features/loans";

const API_FILES = "http://localhost:5000"; // mismo patrón de UserViewLeft
const fmt = (d) => (d ? String(d).slice(0, 10) : "—");

export default function LoanReturnLeft({ loan }) {
  const receiver = loan?.signatures?.find((s) => s.party === "Receptor")?.user;
  const receiverName = receiver ? `${receiver.userFirstName} ${receiver.userLastName}` : "—";
