...
export function generateExcelReport({
    ...
    fileName = "returnable-report.xlsx" // Nombre del archivo de salida

}){

    const currentDate = new Date().toLocaleDateString();
    const reportTitle = `---------- REPORTE DE MATERIALES RETORNABLES - ${currentDate} ----------`
    ...
    // Merge visual del título sobre todas las columnas
    ...
    // Ancho de columnas
    worksheet["!cols"] = headers.map(() => ({ wch: 40 }));
    ...
    // Agrega la hoja al libro con el nombre "Retornables"
    XLSX.utils.book_append_sheet(workbook, worksheet, "Retornables");
